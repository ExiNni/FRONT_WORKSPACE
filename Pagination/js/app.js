const images = [
  ...
  let currentPage = 0; // 현재 페이지를 나타내는 변수 초기값은 0으로 설정 첫 번째 페이지를 표시함
  
  const imageContainer = document.querySelector('.img-container'); // 이미지가 표시된 div공간
  ...
  */
  function displayImg(page){
      ...
      // 이밎 배열('images')에 시작 인덱스와 종료 인덱스 사이에 이미지를 추출해서
      // pagesImages 배열에 저장 이 배열은 현재 페이지에 표시될 이미지들을 담고 있음
  ...
  function updateButtons(){
      /*
          ...
          첫 번째 페이지 일 경우에는 이전 버튼을 클릭하지 못하게 막는다
      */
      
      prevButton.disabled = currentPage ===0;
     
     /*
       nextButton.disabled = 다음 버튼에서 마지막 페이지일 경우 이미지가 없을 때
       다음 버튼이 눌리는 것을 방지하기 위해서 다음 버튼 또한 클릭하지 못하게 막음
       (currentPage + 1) * imagesPerpage= 다음 베피이제 해당하는 이미지의 첫 번째 인덱스
       images.length = 페이지를 넘어갈 수 있는 이미지의 전체 수를 나타냄
       nextButton.disabled =(currentPage + 1) * imagesPerpage >= images.length;
       다음 페이지에 해당하는 이미지가 이미지 배열을 벗어나는지 확인
       다음 페이지에 이미지가 더 이상 없을 때 다음 버튼을 비활성화 하겠음
      */
      nextButton.disabled =(currentPage + 1) * imagesPerpage >= images.length;
  }
  ...
  prevButton.addEventListener('click', ()=>{
      if(currentPage >0){ // 현재 페이지 번호가 0보다 클 경우에만 실행
          ...
          updateButtons(); // 이전 버튼이 첫 번째 페이지에서 클릭하게 되면
  ...
  nextButton.addEventListener('click', () =>{
      ...
      /*
          ...
          imagesPerpage: 한 페이지당 표시되는 이미지 수
